...
def download_report(job_id, fmt):
    ...
    content = builder(data["target"], data["findings"], data["meta"])
    mimetypes = {
...
def _load_findings(job_id):
    """Resolve a job's findings/meta/target from the in-memory job table
    first (fresh from this process), falling back to the SQLite scan
    history (survives a server restart)."""
    job = JOBS.get(job_id)
    if job:
        return {"target": job.get("target", ""), "findings": job.get("findings", []), "meta": job.get("meta", {})}

    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(
        "SELECT target, report_json FROM scan_history WHERE job_id = ?", (job_id,)
    ).fetchone()
    conn.close()
    if not row:
        return None
    report = json.loads(row[1])
    return {"target": row[0], "findings": report.get("findings", []), "meta": report.get("meta", {})}
...
def history_delete(job_id):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("DELETE FROM scan_history WHERE job_id = ?", (job_id,))
    conn.commit()
    conn.close()
...
def diff_route():
    data = request.get_json(force=True)
    job_a, job_b = _load_findings(data.get("job_a", "")), _load_findings(data.get("job_b", ""))
...
def main():
    """Console entry point (the `mass-pii-finder` command) and the target of
    the root-level `python app.py` shim. Launches the local GUI."""
    print("\n  Mass PII / Secret Finder GUI")
